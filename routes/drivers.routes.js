...
module.exports = function crearRutasDrivers({ pool }) {
  ...
  r.patch('/:driverId/jobs/:jobId/status', async(req,res)=>{
    ...
    const client=await pool.connect(); try {await client.query('BEGIN'); const job=(await client.query('SELECT * FROM bhuz_delivery_jobs WHERE id=$1 AND driver_id=$2 FOR UPDATE',[req.params.jobId,req.params.driverId])).rows[0];
      ...
      if(job.source_type==='FOOD_ORDER') await client.query(`UPDATE orders SET delivery_status=$2,status=CASE WHEN $2='PICKED_UP' THEN 'retirado' WHEN $2='GOING_TO_DELIVERY' THEN 'en_camino' WHEN $2='DELIVERED' THEN 'entregado' ELSE status END,picked_up_at=CASE WHEN $2='PICKED_UP' THEN COALESCE(picked_up_at,NOW()) ELSE picked_up_at END,en_route_at=CASE WHEN $2='GOING_TO_DELIVERY' THEN COALESCE(en_route_at,NOW()) ELSE en_route_at END,delivered_at=CASE WHEN $2='DELIVERED' THEN COALESCE(delivered_at,NOW()) ELSE delivered_at END,updated_at=NOW() WHERE id=$1`,[job.source_id,next]);
      if(delivered){
  ...
  r.post('/:driverId/jobs/:jobId/confirm-delivery', async(req,res)=>{
    const deliveryCode=String(req.body?.deliveryCode||req.body?.code||'').replace(/\D/g,'').slice(0,6);
    if(deliveryCode.length!==6) return res.status(400).json({ok:false,message:'Ingresa los 6 dígitos del código de entrega.'});
    ...
    try{
      ...
      if(job.source_type==='PACKAGE'){
        ...
        await client.query(`UPDATE orders SET status='entregado',delivery_status='DELIVERED',delivery_code_used=TRUE,delivered_at=COALESCE(delivered_at,NOW()),updated_at=NOW() WHERE id=$1`,[job.source_id]);
      }
      ...
      await client.query(`UPDATE bhuz_drivers SET operational_status='AVAILABLE',is_available=TRUE,completed_deliveries=completed_deliveries+1,updated_at=NOW() WHERE id=$1`,[req.params.driverId]);
      await client.query('COMMIT');
      res.json({ok:true,message:'Entrega confirmada correctamente.',job:q.rows[0]});
    }catch(err){await client.query('ROLLBACK');console.error(err);res.status(500).json({ok:false,message:err.message||'No se pudo confirmar la entrega.'});}
